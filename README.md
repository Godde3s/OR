# OR — Reza Bazdar (Godde3s) Portfolio

Personal portfolio of **Reza Bazdar** — Creative Software Engineer.

Live: https://godde3s.github.io/OR/

## About

A single-page portfolio built with plain HTML, CSS and vanilla JavaScript — no frameworks, no build step. The visual direction is inspired by the clean, notebook-style product aesthetic of [deepnote.com](https://deepnote.com) (Inter typography, purple-on-white palette, notebook cell motif); all code and content are original.

## Sections

- Hero
- Project stories — MacBook slider (ported from the Usf portfolio)
- Selected projects — every card links to a working public repository
- Skills toolbox
- Agentic coding stack (OpenCode, Codex, Claude Code, Hermes)
- About + stats
- Contact CTA
- Interactive notebook playground — press **Run all**

## Stack

- HTML5 + CSS3 (custom properties, grid, responsive)
- Vanilla JS (menu, scroll reveal, notebook interaction)
- GitHub Pages

Previous portfolio: [godde3s.github.io/Usf](https://godde3s.github.io/Usf/)
